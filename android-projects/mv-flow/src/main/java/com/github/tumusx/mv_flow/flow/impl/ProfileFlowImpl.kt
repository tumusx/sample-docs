...
class ProfileFlowImpl : IFlow {

    private val _infoNavigation: MutableStateFlow<InfoNavigation?> = MutableStateFlow(null)

    override val infoNavigation: Flow<InfoNavigation?> = _infoNavigation.asStateFlow()

    override suspend fun onNavigateToRoute() {
        _infoNavigation.value = InfoNavigation(Actions.GoToProfile)
    }
}